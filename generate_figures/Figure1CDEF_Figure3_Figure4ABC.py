...
for figure_idx in range(2):
    ...
    model_cols = [0,3,4,5]
    if figure_idx == 0:
        model_names = ['fly','LN_highestnoise','LNLN_highestnoise','conductance_highestnoise']
    else:
        model_names = ['fly','LN_highestnoise','sine_highestnoise','direction_highestnoise']
    ...
    for model_idx, model_type in enumerate(model_names):
        ...
        D = sio.loadmat(path + model_type +'.mat')
    ...
    figure_name = "Figure1CDEF_Figure3" if figure_idx == 0 else "Figure4ABC"
    fig.savefig(os.path.join(output_path,figure_name + ".pdf"), bbox_inches='tight')
