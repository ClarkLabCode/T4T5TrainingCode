...
injectedNoises = [0,0.125,0.125,1    ,1];
trainedNoises =  [0,0.125,1    ,0.125,1];
titles = {'0 Trained, 0 Eval','0.125 Trained, 0.125 Eval','1 Trained, 0.125 Eval','0.125 Trained, 1 Eval','1 Trained, 1 Eval'};
numModels = length(injectedNoises);
...
for modelIdx = 1:numModels
    ...
    subplot(5,numModels,modelIdx);
    tt = linspace(0,720,72);
    dataSegment = data_set_noised(end-72+1:end,1:3,chosenTrace);
    plot(tt,dataSegment);
    xlabel('time (ms)')
    ...
    subplot(5,numModels,numModels+modelIdx);
%     A = component_output{1}(:,1,chosenTrace) + component_output{3}(:,1,chosenTrace);
%     B = component_output{2}(:,1,chosenTrace) + component_output{4}(:,1,chosenTrace);
    component_output_traces = [];
    for ii = 1:4
        component_output_traces = cat(2,component_output_traces,component_output{ii}(:,1,chosenTrace));
    end
    plot(tt,[data_ans(:,1,chosenTrace),model_output(:,1,chosenTrace), component_output_traces])
    xlabel('time (ms)');
    ...
    subplot(5,numModels,2*numModels+modelIdx);
    hold on;
    ...
    plot(tt,data_ans(:,1,chosenTrace),'b');
    ...
    subplot(5,numModels,3*numModels+modelIdx);
    truth = data_ans(1,1,:);
    guess = model_output(1,1,:);
    plot(truth(:),guess(:),'.','Color',[0.8, 0.8, 0.8]);
%     hold on;
%     [x,y,e] = calcFitLine(truth(:),guess(:));
%     plotErrorBands(x,y,e);
%     hold off;
    num = sum((truth(:)-guess(:)).^2);
    denom = sum((truth(:)-mean(truth(:))).^2);
    varExpl = 1-mean(num./denom);
    R = corrcoef(truth(:),guess(:));
    rsqr = R(2)*R(2);
    title(['VarExpl: ' num2str(varExpl) ', r^2: ' num2str(rsqr)]);
    xlabel('True Velocity');
    ylabel('Model Output');
    
    subplot(5,numModels,4*numModels+modelIdx);
    truth = data_ans(1,1,:);
    guess = mean(model_output(1,:,:),2);
    plot(truth(:),guess(:),'.r');
%     hold on;
%     [x,y,e] = calcFitLine(truth(:),guess(:));
%     plotErrorBands(x,y,e);
