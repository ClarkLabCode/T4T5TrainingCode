function [model_output, component_output, numerators, denominators] = conductance_model(data_set,h,b1,m1,b2,m2)
    ...
    for h_ind = 1:size(h,3)
        ...
        denominator = g_leak + sum(channels,4);
        component_output{h_ind} = numerator./denominator;
        ...
        if nargout > 2
            numerators{h_ind} = gather(numerator);
            denominators{h_ind} = gather(denominator);
        end
    end
